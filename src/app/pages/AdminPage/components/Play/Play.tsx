...
const Play: FC<PlayProps> = (props) => {
  return (
    <section key={props.player.id} className="grid content-between grid-cols-3 pb-8 text-center font-bold hover:text-light ease-in-out duration-200;">
      <span className='text-white place-self-end pb-4'>
        <span className='mr-8'>{props.player.name}</span>
      </span>
      <span className='text-white place-self-end pb-4'>
